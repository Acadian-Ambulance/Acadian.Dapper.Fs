...
let buildVersion =
  lazy
    (if BuildServer.isLocalBuild then
       let input =
         UserInput.getUserInput "Package Version: "
...
let nugetApiKey =
  lazy
    (match BuildServer.buildServer with
     ...
     | _ -> Environment.environVarOrFail "NugetPublishApiKey")
...
let setVersion (a: MSBuild.CliArguments) =
  ...
  { a with
      Properties = props |> List.map (fun p -> p, buildVersion.Value) }
...
Target.create
  ...
  (fun _ ->
    let path =
      sprintf "nupkgs/%s.%s.nupkg" packageProject buildVersion.Value

    let pushParams (p: NuGetPushParams) =
      { p with
          Source = Some "https://api.nuget.org/v3/index.json"
          ApiKey = Some nugetApiKey.Value }

    DotNet.nugetPush
      (fun o ->
        { o with
            PushParams = o.PushParams |> pushParams })
      path)
